...
void MI2C_voidInit(void)
{
	...
	MI2C_TWCR=Set_Bit(MI2C_TWCR,TWEN);//enable I2C module

	MI2C_TWDR=8;		//set TWBR=8 and set TWPS 0 and 0

	MI2C_TWSR=Clr_Bit(MI2C_TWSR,TWPS0);	// to make the clock is 100 khz
	MI2C_TWSR=Clr_Bit(MI2C_TWSR,TWPS1);
...
void MI2C_voidStartCondition(void)
{
	...
	MI2C_TWCR=Set_Bit(MI2C_TWCR,TWSTA);//send start condtion
...
u8 MI2C_u8ReceiveDataWithNoAck(u8 CopySlaveAddress)
{
	MI2C_TWCR= (1<<TWINT)  |  (1<<TWEN);
